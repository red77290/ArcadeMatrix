...
- `[TIME]` : fuseau horaire, options de mise en page de l'horloge, et couleurs de dégradé `clock_color_1`/`clock_color_2`.
- `[IDLE]` : séquence des modules à jouer quand rien ne se passe (Clock, Date, Weather, GIFs, Sprites), incluant `fighter_interval_sec` (délai entre les combats MUGEN).
