...
echo    * 0.5   : Recommande ESP32 Matrice 128x32 / 64x32
echo    * 1.0   : Taille 1:1 d'origine (Matrice 128x64, 256x64, RPi)
echo    * auto  : Ajustement automatique proportionnel
echo    * Ou entrez une valeur personnalisee (ex: 0.4, 0.75, 1.5)
echo -----------------------------------------------------
set "scale_input="
set /p scale_input="Echelle souhaitee [default: 0.5]: "
if "%scale_input%"=="" set "scale_input=0.5"

echo.
echo -----------------------------------------------------
echo Compression des fichiers (.fgt vs .fgt.gz) :
echo    * n (Non) : Recommande pour ESP32 / SD-Card / LittleFS
echo    * y (Oui) : Recommande pour Raspberry Pi (-80%% espace)
echo -----------------------------------------------------
set "compress_input="
set /p compress_input="Compresser en .fgt.gz ? (y/n) [default: n]: "
